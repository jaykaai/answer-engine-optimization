...
# article -> list of (svg_source_file_in_new, png_slug) in document order
JOBS = {
...
for art, pairs in JOBS.items():
    ...
    # step 1: put the final corrected SVG from /tmp/dd/new into the article.
    # Iterate over indices right-to-left so earlier offsets stay valid.
    ...
    for nf, slug in pairs:
        svg = open(NEW + "/" + nf, encoding="utf-8").read().strip()
        w, h = (int(x) for x in VB_RE.search(svg).group(1).split())
        tmp = TMPDIR + "/" + slug + ".html"
        ...
        rows.append((slug, w, h, pw, ph, os.path.getsize(png)))

    # step 3: swap <svg>...</svg> -> <img>, right-to-left.
    t = open(path, encoding="utf-8").read()
...
print("png".ljust(32), "viewBox".ljust(14), "pixels".ljust(14), "bytes")
for slug, w, h, pw, ph, n in rows:
    print(slug.ljust(32),
          ("%dx%d" % (w, h)).ljust(14),
          ("%dx%d" % (pw, ph)).ljust(14), n)
print("\nexported %d pngs, swapped %d articles" % (len(rows), len(JOBS)))

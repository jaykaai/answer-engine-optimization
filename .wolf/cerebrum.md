...
- **[2026-08-30] 配图偏好：SVG 显示不出来，改用终端字符流程图**。用户明确反馈"SVG 没办法直接显示，要终端可以直接显示的"。**决策：文章中讲原理/流程的图，一律用纯字符流程图（ASCII/Unicode 框线图）写在 fenced code block（```）里**——任何终端/编辑器/浏览器都直接显示，marked 渲染成 `<pre><code>` 保留等宽排版。不再用 SVG（在用户的终端/预览环境渲染不出来）。字符图要点：横向流程图（左→右），框线用 ┌─┐│└┘─▶ 等 Unicode 字符，标注精简（标题 + 各阶段一两行说明 + 底部结论）。这是对"苹果风手绘 SVG 铁律"的重要修正——那条铁律适用于"能渲染 SVG 的场景"，但用户实际环境显示不了 SVG，优先保证"能直接看"。

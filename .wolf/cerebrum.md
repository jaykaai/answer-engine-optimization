...
- **[2026-09-03] AutoGEO 架构学习**：AutoGEO（ICLR 2026, arXiv 2510.11438）三组件：①规则提取（`rules/extractor.py`：让 LLM 解释 pairwise 比较中为何偏好 A 胜 B → 提炼可复用规则，JSON 数组）②AutoGEO_API（`rewriters/core.py`：把规则格式化为 Quality Guidelines 拼进 user_prompt，调 Gemini-2.5-pro 重写；无规则文件时降级到 9 组默认规则 = 3 数据集 × 3 引擎，规则是引擎特异+领域特异）③AutoGEO_Mini（SFT cold start via LLaMA-Factory → GRPO via open-r1，2× A100, ~52h）。GEO 分数公式（`evaluation/metrics/geo_score.py`）：`词数 × exp(-位置/总句数) ÷ 该句引用数`，三维度=词数+位置(指数衰减)+引用频率，归一化为比例。GEU 分数=实用性（引用质量+关键点覆盖+回答质量）。重写提示核心定义 GEO 本质："You are the owner of the source... rewrite your document to maximize its visibility in the LLM's final answer"。用户之前学的 47 种方法 ≈ AutoGEO 默认规则的手工版本。学习计划见 `cn/学习/AutoGEO学习计划.md`（6 阶段 A-F，24 课，A 全局认知/B GEO 分数/C 规则提取/D AutoGEO_API/E AutoGEO_Mini/F 变现收束）。
...
- **[2026-09-03] "为什么"和"怎么做"都要讲（偏好修正）**：用户纠正"不，既要讲为什么，也要讲怎么做"。之前 cerebrum 记的是"要'为什么'层面的原理，不要只背'做什么'"——这不意味着只讲为什么。**正确理解：不要"只背做什么"≠ 不要讲做什么。两者都要：先讲透"为什么"（原理/动机/机制），再给"怎么做"（具体操作/代码/工具）。缺哪个都不行——只有为什么没有怎么做=空谈，只有怎么做没有为什么=背清单。** 后续 AutoGEO 教学每课都要两层都覆盖。
- **[2026-09-03] "怎么做"= 我解释代码做了什么，不是用户动手跑（关键澄清）**：用户第三次纠正"我不是需要动手实际操作，我是需要你告诉我他做了什么"。之前我把"操作层"写成让用户跑命令（如"跑 `bash install.sh`"、"调 `rewrite_document()`"）——错了。**正确理解：用户是学习者/接收者，不是执行者。所谓"操作方法"= 我（Claude）在教学中向用户解读 AutoGEO 代码内部具体做了什么操作、调了什么、数据怎么流转、机制是什么。** 用户不动手跑代码。学习计划每课的"操作层"已改为"代码行为解读"（逐行/逐函数讲清 AutoGEO 内部做了什么），而非"用户执行指令"。AutoGEO_Mini 训练需 2× A100 + ~52h，用户本地更不可能跑——原理靠我讲透即可。
...
- **[2026-09-08] "引用率检测"功能原理（横向研究 7 个开源 GEO 项目）**：本质 = 批量向 AI 引擎提问 + 解析回答结构化字段 + 实体匹配 + 比率聚合。三条采集通道：①**官方 API**（geolook `sample.py:ask()`，各家来源字段不同：千问 `search_info.search_results`、Perplexity `citations`/`search_results`）②**浏览器扩展 DOM 提取**（geolook `extension/content.js`：按 host 匹配 `ANSWER_SELECTORS`，取"最后一个可见元素中文本最长者"为答案容器，`<a href>` 收集引用并**排除同域链接**；选择器失效降级到"用户选中文本"，这是最可靠的兜底）③**无 API 引擎用人工/半自动采样**。四条指标公式：`mention_rate = 被提及样本数 / 总样本数`；`citation_rate = 你的域名出现在引用列表的次数 / 总查询数`；`SoV = 品牌提及 / (品牌提及 + 竞品提及)`；`citation_share = 品牌引用 / 总引用`。关键纪律：①**点名题必须隔离**（geolook `brand_in_question()`：问题里含品牌名/域名 → 答案必然复述品牌名，提及率 100% 是假阳性，单独归"品牌认知"，绝不回退混算）②**否定语境检查**（`_NEG_RE` 在命中所在句找"不是/并非/not"→ 不算提及但标 needs_review）③**词边界**（拉丁侧加 `(?<![A-Za-z0-9])` 防 "AIGC" 命中 "AIGCLINK"；CJK 侧不加，中文无空格分词）④**跨文种边界不算词延续** ⑤**负向窗口**（品牌命中点前 80/后 160 字符找负面词，只标"疑似"进人工复核）⑥**采样卫生**（sandbox 无痕浏览器/独立 profile，记录 provenance：sandbox/incognito/dedicated/personal，personal 自动降级"needs review"）⑦**LVCF 平滑**（getcito `applyPerPromptLVCF`：每 prompt 最后一个已知值向未来日结转再按天求和，避免不同 prompt 排期错峰把折线锯成锯齿）。两套范式区分：①**主动实测型**（geolook、getcito、geo-optimizer-skill `citations.py`）= 真问真答，数字真实；②**被动就绪度型**（geo-optimizer-skill `monitor.py`，不调用任何 AI，只审自己站点 robots.txt/llms.txt/实体/trust stack，7 信号加权：citation_bot_access 20 + user_fetch 10 + llms 15 + ai_discovery 15 + entity 15 + trust 15 + momentum 10，CITATION_BOTS={OAI-SearchBot, Claude-SearchBot, PerplexityBot, Googlebot, Applebot}——这是**代理指标不是引用率**）③**学术型**（AutoGEO `geo_score.py`：`词数 × exp(-位置/总句数) ÷ 该句引用数`，衡量"我的文档在被引用时权重多大"，不衡量"是否被引用"）。getcito `web_queries` 字段（`server_tool_use` 里 web_search 的 input.query）可导出"引擎回答时实际搜了什么词"，用于 Query Fan-out 分析，比猜搜索词更准。
